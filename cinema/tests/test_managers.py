...
class TestCinemaPersonManager:
    def test_get_brief_data(self, cinema_person_factory):
        ...
        expected_data = [
            {'pk': person_1.pk, 'user__first_name': person_1.user.first_name,
             'user__last_name': person_1.user.last_name},
            {'pk': person_2.pk, 'user__first_name': person_2.user.first_name,
    ...
    def test_filter_by_search_word(self, cinema_person_factory):
        ...
        cinema_person_factory(
            user__first_name="Tom", user__last_name="Hanks"
        )
        cinema_person_factory(
            user__first_name="Al", user__last_name="Pacino"
        )
        cinema_person_factory(
            user__first_name="Tom", user__last_name="Cruise"
        )
    ...
    def test_get_related_data(
            self, cinema_film_person_profession_factory, cinema_person_factory,
            news_factory, film_factory, genre_factory, cinema_profession_factory
    ):
        ...
        cinema_film_person_profession_factory(
            cinema_person=person, film=film, profession=profession
        )
        news = news_factory(cinema_person=(person, ), film=(film, ))
        expected_data = [
            {"pk": person.pk, "film__genre__name": genre.name,
             "news__pk": news.pk, "news__title": news.title},
        ]
...
class TestFilmManager:
    ...
    def test_filter_by_selected_year(self, film_factory):
        film_factory.create_batch(
            5, release_data=date(year=2000, month=1, day=1)
        )
        film_factory.create_batch(
            3, release_data=date(year=2020, month=1, day=1))
    ...
    def test_filter_by_selected_distributor(self, film_factory, distributor_factory):
    ...
    def test_filter_by_selected_mpaa_rating(self, film_factory, mpaa_rating_factory):
        mpaa_rating_1 = mpaa_rating_factory(
            value="Not Rated", description="The upcoming movie"
        )
        mpaa_rating_2 = mpaa_rating_factory(
            value="R", description="Children under 17"
        )
    ...
    def test_get_related_data(
            self, film_factory, country_factory, genre_factory,
            language_factory, news_factory
    ):
        ...
        film_1 = film_factory(
            country=(country_1, ), genre=(genre_1, ), language=(language_1, )
        )
        film_2 = film_factory(
            country=(country_2, ), genre=(genre_2, ), language=(language_2, )
        )
        news_1 = news_factory(film=(film_1, ))
        news_2 = news_factory(film=(film_2, ))
        expected_data = [
            {"pk": film_1.pk, "country__name": country_1.name,
             "genre__name": genre_1.name, "language__name": language_1.name,
             "news__title": news_1.title},
            {"pk": film_2.pk, "country__name": country_2.name,
             "genre__name": genre_2.name, "language__name": language_2.name,
...
class TestCinemaFilmPersonProfessionManager:
    def test_get_full_cast(
            self, cinema_person_factory, cinema_profession_factory,
            film_factory, cinema_film_person_profession_factory
    ):
        person_1 = cinema_person_factory()
        person_2 = cinema_person_factory()
        ...
        film_2 = film_factory()
        cinema_film_person_profession_factory(
            cinema_person=person_1, film=film_1, profession=profession_1
        )
        cinema_film_person_profession_factory(
            cinema_person=person_2, film=film_2, profession=profession_2
        )
        expected_data = [
            {"film__pk": film_1.pk, "profession__name": profession_1.name,
            ...
            {"film__pk": film_2.pk, "profession__name": profession_2.name,
