...
DESCRIPTION_BLU_RAY = (
    "Welcome to the fantastic Blu-ray department here, "
...
def product_detail(request, pk):
    ...
    Also display comments to blue-ray movie and form for adding new
    comment. Create "CommentToProduct" model record. After saving data,
...
class BudgetFilmListView(FilmListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context.update(
            {"page_title": "Most Expensive Movies", "criterion_name": "Budget"}
        )
        return context
...
class GenreFilmListView(FilmListView):
    ...
    def get_queryset(self):
        ...
        return Film.films.filter_by_selected_genre(self.requested_genre)
...
class CountryFilmListView(FilmListView):
    ...
    def get_queryset(self):
        ...
        return Film.films.filter_by_selected_country(self.requested_country)
...
class LanguageFilmListView(FilmListView):
    ...
    def get_queryset(self):
        ...
        return Film.films.filter_by_selected_language(self.requested_language)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["page_title"] = (
            f"Most Popular Movies in " f"{self.requested_language} "
        )
        return context
...
class DistributorFilmListView(FilmListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["page_title"] = (
            f"Most Popular Movies of " f"{self.requested_distributor}"
        )
        return context
...
class MpaaFilmListView(FilmListView):
    ...
    def get_queryset(self):
        ...
        return Film.films.filter_by_selected_mpaa_rating(self.requested_mpaa)
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "page_title": f"Most Popular Movies with MPAA Rating of {mpaa.value}",
...
def film_detail(request, pk):
    ...
    Create "CommentToFilm" model record. After saving data, redirect to
    current page.
    """
...
def cinema_person_detail(request, pk):
    ...
    Also display comments to cinema person and form for adding new
    comment. Create "CommentToPerson" model record. After saving data,
    redirect to current page.
    """
...
def news_detail(request, pk):
    ...
    Create "CommentToNews" model record. After saving data, redirect to
    current page.
    """
...
class SearchResultsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if cleaned_search_word:
            ...
            film_list = Film.films.filter_by_search_word(cleaned_search_word)
            person_list = CinemaPerson.persons.filter_by_search_word(
                cleaned_search_word
            )
            news_list = News.news.filter_by_search_word(cleaned_search_word)
            context = {
